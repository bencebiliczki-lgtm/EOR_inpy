...
A V3 nyers CSV a vonali és differenciálnyomásnál külön
`raw_*_pressure_bar` és szűrt nyomásoszlopot tart fenn. A V1/V2 fájlok
megnyitáskor biztonsági másolat mellett migrálódnak; korábbi fájlnál a régi
...
nem olvadnak össze. A belső **Grafikon/Táblázat** lapváltó ugyanazokat a szűrt
indexeket használja. A táblázat fejléce a nyers CSV és az Excel közös fejlécéből
származik, az UTC időpontot csak megjelenítéskor váltja magyar időre, és
`QAbstractTableModel` segítségével legfeljebb 1000 soros lapokat ad át a Qt-nak.
...
Az alkalmazás a tartós mérési, projekt- és diagnosztikai időbélyegeket UTC-ben
őrzi. A kezelőfelület és a HTML diagnosztikai nézet ezeket az
`Europe/Budapest` zónában jeleníti meg; a projektkönyvtár dátuma és éve is a
magyar helyi naptári napból származik. A zónainformációt az alkalmazáscsomagba
épített `tzdata` biztosítja, így a téli/nyári időszámítás az offline célgépen is
követhető.
...
Az NI analóg bemenet egy vezérlési ciklusban alapértelmezetten 20 mintás, 1 kHz-es
véges burstöt olvas, legfeljebb 100 ms-os hardveres várakozással. A feldolgozási
lánc csatornánként: medián, egyszeri nagy ugrás visszatartása, EMA (`alpha=0,2`),
majd lineáris kalibráció. A nyers mediánból és az EMA-kimenetből kalibrált nyomás
egyaránt bekerül a mérési pillanatképbe. A PID, a grafikon és a kijelzés a szűrt
értéket használja; a kemény vonali és differenciálnyomás-határok a nyers értéket
értékelik. A tartós, valódi lépcsőváltozást három egymást követő burst után a
tüskeszűrő is átengedi, de a nyers biztonsági ág már az első burstben aktív.
...
A `PollingPump` mezőnként élérzékelt minőségállapotot tart fenn. A
`GOOD → STALE/INVALID/DISCONNECTED` és visszaállási átmenetek strukturált
eseményként kerülnek a diagnosztikai naplóba. A polling timeout és parse-hiba
külön eseményazonosítót kap; az első hiba után csak periodikus ismétlési
összefoglaló készül.

A `DiagnosticLogger` az aktív alkalmazás- és hardverkommunikációs HTML-naplót
nap- vagy mérethatárnál lezárja. A megőrzési karbantartás külön daemon
háttérszálon tömöríti vagy törli a kizárólag ismert névmintájú lezárt fájlokat.
Az aktív fájlok és a regisztrált védett mérési naplók kizárási listán vannak.
...
a következő indításkor a projekt visszaállítása után automatikusan megnyitja az
